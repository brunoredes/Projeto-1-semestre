...
router.get("/ultimas/:amb", (req, res, next) => {
  ...
  if (ambiente != null) {
    Database.query(
      `SELECT DISTINCT TOP ${limite_linhas} * FROM dado  
...
router.get("/medias", (req, res, next) => {
  Database.query(
    `SELECT avg(vlrTemperatura) as media_temp, avg(vlrUmidade) as media_umid FROM Dado`
  )
    .then(resultados => {
  ...
  if (ambiente != null) {
    Database.query(
      `Select distinct 
                        ...
                        idDado between ${ultimo} AND ${primeiro}`
    )
      .then(resultados => {
        var linha = resultados.recordsets[0][0];
        ...
        res.json({
          ...
          primeiroQTemp: primeiroQTemp,
          terceiroQTemp: terceiroQTemp,
          primeiroQUmi: primeiroQUmi,
          terceiroQUmi: terceiroQUmi
        });
      })
      .catch(error => {
        console.log(error);
...
router.get("/preencheComboBox", (req, res, next) => {
  Database.query(`SELECT txtDescricao FROM ambiente order by txtDescricao`)
    .then(resultados => {
      resultados = resultados.recordsets[0];
      res.json(resultados);
    })
...
router.get("/alerta/:amb", (req, res, next) => {
  ...
  if (ambiente != null) {
    Database.query(
      `SELECT vlrTemperaturaMax, vlrTemperaturaMin, vlrUmidadeMax, vlrUmidadeMin From Ambiente Where txtDescricao = '${ambiente}'`
    )
      .then(resultados => {
        resultados = resultados.recordsets[0];
        res.json(resultados);
